...
class QuestionTable extends Component {
  ...
  render() {
    ...
    if (activeExpansions.length > 0) {
      filtered = filtered.filter(question => activeExpansions.indexOf(question.expansion) > -1);
    }

    if (activeDifficulties.length > 0) {
      filtered = filtered.filter(question => activeDifficulties.indexOf(question.difficulty) > -1);
    }

    if (activeTypes.length > 0) {
      filtered = filtered.filter(question => activeTypes.indexOf(question.type) > -1);
    }

    if (activeCategories.length > 0) {
      filtered = filtered.filter(question => activeCategories.indexOf(question.category) > -1);
    }

    if (FilterStore.search) {
      filtered = filtered.filter(
        question => question.question.toLowerCase().indexOf(FilterStore.search.toLowerCase()) > -1
      );
    }
